...
class alsp_listings_controller extends alsp_frontend_controller {
	public $request_by = 'listings_controller';
	//public $test = '199';
	public function init($args = array()) {
		global $alsp_instance, $ALSP_ADIMN_SETTINGS;
		if (!($template = alsp_isFrontPart('frontend/listings_block.tpl.php'))) {
				$template = 'frontend/listings_block.tpl.php';
		}
			
		$template = apply_filters('alsp_listing_template_block', $template, $this);
...
/* 				'order_by' => (isset($_GET['order_by']) && $_GET['order_by'] ? $_GET['order_by'] : 'post_date'),
		...
		$args = array(
				...
				'paged' => $paged,
				'listings_view_type' => $shortcode_atts['listings_view_type'],
		);
