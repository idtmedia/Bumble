...
function pacz_range_input() {
    jQuery('.pacz-range-input').each(function() {
        var range_input = jQuery(this).siblings('.range-input-selector'),
            pacz_min = parseFloat(jQuery(this).attr('data-min')),
            pacz_max = parseFloat(jQuery(this).attr('data-max')),
            pacz_step = parseFloat(jQuery(this).attr('data-step')),
            pacz_value = parseFloat(jQuery(this).attr('data-value'));
        jQuery(this).slider({
            value: pacz_value,
            min: pacz_min,
            max: pacz_max,
...
function pacz_visual_selector() {
    jQuery('.pacz-visual-selector').find('a').each(function() {
        ...
        jQuery(this).click(function() {
            ...
            jQuery(this).parent('.pacz-visual-selector').find('.selector-tick').remove();
            jQuery(this).addClass('current');
            jQuery(this).append('<div class="selector-tick"></div>');
            return false;
        });
...
function icon_filter_name() {
    jQuery('.page-composer-icon-filter').each(function() {
        jQuery(this).change(function() {
            ...
            var list = jQuery(this).siblings('.pacz-font-icons-wrapper');
            if (filter) {
...
function pacz_composer_preview_button() {
...
/* 
**
Sharp Slideshow functions
-------------------------------------------------------------*/
function pacz_sharp_slideshow() {
    pacz_sharp_choices = jQuery('#_video_mp4_wrapper, #_video_ogv_wrapper, #_video_webm_wrapper, #_video_preview_wrapper, #_slide_image_wrapper');
    pacz_sharp_choices.hide();
    sharp_source_val = jQuery('#_sharp_type').val();
    if (sharp_source_val == 'image') {
        jQuery('#_slide_image_wrapper').show();
    } else if (sharp_source_val == 'video') {
        jQuery('#_video_mp4_wrapper, #_video_webm_wrapper, #_video_preview_wrapper, #_video_ogv_wrapper').show();

    }

    jQuery('#_sharp_type').change(function() {
...
-------------------------------------------------------------*/
function pacz_transparent_header() {
    pacz_choices = jQuery('#_trans_header_offset_wrapper, #_trans_header_skin_wrapper');
    pacz_choices.hide();
...
-------------------------------------------------------------*/
function pacz_transparent_header() {
    pacz_choices = jQuery('#_quick_contact_skin_wrapper');
    pacz_choices.hide();
    header_source_val = jQuery('#_header_style').val();
    if (header_source_val == 'transparent') {
        pacz_choices.show();
...
jQuery(document).ready(function() {
    ...
    pacz_toggle();
    pacz_color_picker();
    pacz_visual_selector();
    pacz_composer_preview_button();
    pacz_sharp_slideshow();
    pacz_transparent_header();

    jQuery(".pacz-chosen").each(function() {
        jQuery(this).chosen({
    ...
    pacz_background_orientation = jQuery('#background_selector_orientation').val();



    /* update background viewer accordingly */
    jQuery('.pacz-general-bg-selector').addClass(jQuery('#background_selector_orientation').val());
    jQuery('.background_selector_orientation a, #background_selector_orientation_container a').on('click', function() {
        if (jQuery(this).attr('rel') == 'full') {
            jQuery('.pacz-general-bg-selector').removeClass('boxed').addClass('full');
        } else {
            jQuery('.pacz-general-bg-selector').removeClass('full').addClass('boxed');
            body_section_width = jQuery('.pacz-general-bg-selector .outer-wrapper').width();
            jQuery('.pacz-general-bg-selector.boxed_layout .body-section').css('width', body_section_width);
        }
    ...
    function call_background_edit() {
        ...
        sections.each(function() {
            jQuery(this).on('click', function(event) {
                ...
                jQuery('#pacz-bg-edit-panel').fadeIn(200);
                ...
                jQuery('#pacz-bg-edit-panel a[rel="' + position_value + '"]').siblings().removeClass('selected').end().addClass('selected');
                jQuery('#pacz-bg-edit-panel a[rel="' + repeat_value + '"]').siblings().removeClass('selected').end().addClass('selected');
                jQuery('#pacz-bg-edit-panel a[rel="' + attachment_value + '"]').siblings().removeClass('selected').end().addClass('selected');
                ...
                jQuery('#pacz-bg-edit-panel').attr('rel', jQuery(this).attr('rel'));
                jQuery('#pacz-bg-edit-panel').find('.pacz-edit-panel-heading').text(jQuery(this).attr('rel'));

                jQuery('.bg-background-type-tabs').find('a[rel="' + source_value + '"]').parent().siblings().removeClass('current').end().addClass('current');


                jQuery('#pacz-bg-edit-panel').find('.bg-background-type-panes').children('.bg-background-type-pane').hide();
                if (source_value == 'no-image') {

                    jQuery('#pacz-bg-edit-panel').find('.bg-background-type-pane.bg-no-image').show();

                } else if (source_value == 'custom') {

                    jQuery('#pacz-bg-edit-panel').find('.bg-background-type-pane.bg-edit-panel-upload').show();
                }



                jQuery('#pacz-bg-edit-panel').find('.bg-background-type-tabs a').on('click', function(event) {
    ...
    jQuery('#pacz_cancel_bg_selector, .pacz-bg-edit-panel-heading-cancel').on('click', function(event) {
        event.preventDefault();
        jQuery('#pacz-bg-edit-panel').fadeOut(200);
    });

    /* Triggers cancel button for background panel when escape key is pressed */
    jQuery(document).keyup(function(e) {
        if (e.keyCode == 27) {
            jQuery('#pacz_cancel_bg_selector, .pacz-bg-edit-panel-heading-cancel').click();
        }
    });
    ...
    jQuery(document).keyup(function(e) {
        if (e.keyCode == 13) {
            jQuery('#pacz_apply_bg_selector').click();
        }
    });

    /* Sends Panel Modifications into inputs and updates preview panel background */
    function update_panel_to_preview() {
        jQuery('#pacz_apply_bg_selector').on('click', function(event) {
            event.preventDefault();
            panel = jQuery('#pacz-bg-edit-panel');
            ...
            if (image != '') {
                jQuery(section_preview_class).find('.pacz-bg-preview-layer').css({
    ...
    function update_preview_on_load() {
        ...
        jQuery('.body-section, .page-section, .header-section, .footer-section, .banner-section').each(function() {
            ...
            //update preview panel background
            if (image != '') {
                jQuery(this_panel).find('.pacz-bg-preview-layer').css({
                    'background-image': 'url(' + image + ')',
                });
            }
