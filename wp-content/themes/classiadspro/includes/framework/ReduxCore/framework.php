    ...
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        class ReduxFramework {
            ...
            public static function init() {
                $dir = Redux_Helpers::cleanFilePath(trailingslashit( get_template_directory() ).'includes/framework/ReduxCore');
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'classiadspro' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'classiadspro' );
                }
            ...
            private function _internationalization() {
                 ...
                 * @param string     'classiadspro'  text domain
                 */
                //                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'classiadspro' );
                //
                ...
                $loaded = load_plugin_textdomain( 'classiadspro', false, $basepath . 'languages');

                if ( !$loaded ){
                    $loaded = load_muplugin_textdomain( 'classiadspro', $basepath . 'languages' );
                }

                if ( !$loaded ){
                    $loaded = load_theme_textdomain( 'classiadspro', $basepath . 'languages' );
                }

                if ( ! $loaded ) {
                    $locale = apply_filters( 'plugin_locale', get_locale(), 'classiadspro' );
                    $mofile = dirname( __FILE__ ) . '/languages/classiadspro-' . $locale . '.mo';
                    load_textdomain( 'classiadspro', $mofile );
                }
            }
            ...
            private function add_submenu( $page_parent, $page_title, $menu_title, $page_permissions, $page_slug ) {
                ...
                if ( isset ( $test[ $page_parent ] ) ) {
                    ...
                    if ( true == $addMenu ) {
                        ...
                        $this->page = call_user_func( $add_submenu.'_page', $page_parent, $page_title, $menu_title, $page_permissions, $page_slug, array(
                            &$this,
                            'generate_panel'
                        ) );
                    }
            ...
            public function _options_page() {
                ...
                if ( $this->args['menu_type'] == 'hidden' ) {
                    ...
                    $this->page = call_user_func( 'add_'.$menu.'_page', $this->args['page_title'], $this->args['menu_title'], $this->args['page_permissions'], $this->args['page_slug'], array(
                        &$this,
                        'generate_panel'
                    ...
                    if ( true === $this->args['allow_sub_menu'] ) {
                        if ( ! isset ( $section['type'] ) || $section['type'] != 'divide' ) {
                            foreach ( $this->sections as $k => $section ) {
								...
								$submenu = 'submenu';
                                // ONLY for non-wp.org themes OR plugins. Theme-Check alert shown if used and IS theme.
                                call_user_func( 'add_'.$submenu.'_page', $this->args['page_slug'], $section['title'], $section['title'], $this->args['page_permissions'], $this->args['page_slug'] . '&tab=' . $k,
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = __( 'Enable', 'classiadspro' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = __( 'Disable', 'classiadspro' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = __( 'moving the mouse over', 'classiadspro' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'classiadspro' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'classiadspro' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => __( 'Hints', 'classiadspro' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'classiadspro' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'classiadspro' ),
                        'action' => ''
                    ) );
                ...
                if ( ! self::current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'classiadspro' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'classiadspro' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                            ...
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) !== false ) {
                                        $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'classiadspro' );
                    ...
                    if ( ! empty( $this->args['share_icons'] ) ) {
                        foreach ( $this->args['share_icons'] as $idx => $arr ) {
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) !== false ) {
                                        $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'classiadspro' );
