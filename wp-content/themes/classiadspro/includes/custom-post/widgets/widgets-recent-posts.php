...
class Classiadspro_Widget_Recent_Posts extends WP_Widget {

	function __construct() {
		$widget_ops = array( "classname" => "widget_posts_lists", "description" => "Displays the Recent posts" );
		WP_Widget::__construct( "recent_posts", PACZ_THEME_SLUG . " - Recent Posts", $widget_ops );
	...
	function widget( $args, $instance ) {
        ...
        <?php	$image_src_array = wp_get_attachment_image_src( get_post_thumbnail_id(), 'full', true );	
				$image_src = bfi_thumb( $image_src_array[ 0 ], array('width' => 100, 'height' => 100, 'crop'=>true));	
		 ?>
		
		 <img width="100" height="70" src="<?php echo pacz_thumbnail_image_gen($image_src, 100, 100); ?>" alt="<?php the_title(); ?>" />
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions['Classiadspro_Widget_Recent_Posts'] ) )
			delete_option( 'Classiadspro_Widget_Recent_Posts' );
