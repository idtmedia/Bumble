...
function blog_masonry_style( $atts) {
	...
	$output .='<article id="entry-'.get_the_ID().'" class="blog-masonry-entry masonry-'.$item_id.' pacz-isotop-item '.$pacz_column_css.' '.$post_type.'-post-type ' . $item_cat . '">';
	$output .= '<a class="post-type-icon" href="'.get_permalink().'"><i class="pacz-post-type-icon-'.$post_type.'"></i></a>';
	...
	$blog_heading .= '<h2 class="blog-title"><a href="'.get_permalink().'">'.get_the_title().'</a></h2>';
			if($column != 4) {
				ob_start();
				comments_number(__( '0', 'classiadspro' ), __( '1', 'classiadspro' ), __( '%', 'classiadspro' ));
				$blog_heading .= '<a href="'.get_permalink().'#comments" class="blog-comments"><i class="pacz-icon-comment"></i>'.ob_get_clean().'</a>';

				if( function_exists('pacz_love_this') ) {
					ob_start();
					pacz_love_this();
					$blog_heading .= '<div class="pacz-love-holder">'.ob_get_clean().'</div>';
				}
			}
	$blog_heading .= '</div>';
	/***********/
	...
	switch ($post_type) {
		...
		if ( has_post_thumbnail() ) {
			$output .='<div class="featured-image">';
				...
				$output .= '<a title="'.get_the_title().'" href="'.get_permalink().'"><i class="pacz-theme-icon-next-big hover-plus-icon-xsmall"></a></i>';
			$output .='</div>';
		}
			break;
		/***********/
