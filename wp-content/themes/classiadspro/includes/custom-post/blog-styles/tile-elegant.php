...
function blog_tile_elegant_style( $atts, $i) {
	global $post;
	extract( $atts );
	...
	$output .='<article id="entry-'.get_the_ID().'" class="blog-tile-entry tile-elegant tile-'.$item_id.' pacz-isotop-item '.$pacz_column_css.' '.$post_type.'-post-type">';
	...
	$output .='<div class="tile-elegant-content">';
	if($post_type != 'audio') {
		$output .= $blog_heading;
	}
	/* Blog Meta */
