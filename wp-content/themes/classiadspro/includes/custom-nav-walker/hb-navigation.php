<?php
/**
 * Header Builder Navigation walker
 *
 * @todo Move it to HB and fix it later.
 *
 * Warnings: This class should be refactored later. There are too many warnings related to else
 * else statements. Need more time to fix it later.
 *
 ...
 * @return      void
 */
class hb_main_menu extends Walker_Nav_Menu
{
    ...
    function start_lvl(&$output, $depth = 0, $args = array())
    {
        ...
        if (!$this->active_megamenu && $depth != 0) {
            $output .= '<i class="hb-menu-sub-level-arrow">' . Mk_SVG_Icons::get_svg_icon_by_class_name(false, 'mk-icon-angle-right', 16) . '</i>';
        }
    ...
    function end_lvl(&$output, $depth = 0, $args = array())
    {
        ...
        if ($depth === 0) {
            if ($this->active_megamenu) {
                ...
                $output = str_replace("{locate_class}", "hb_mega_col_" . $this->max_columns . "", $output);
                
                foreach ($this->rowsCounter as $row => $columns) {
                    $output = str_replace("{current_row_" . $row . "}", "hb_mega_col_" . $columns, $output);
                }
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0)
    {
        ...
        if ($depth === 0) {
            ...
            if ($this->active_megamenu) {
                $column_class .= " hb-has-mega-menu";
            } else {
                $column_class .= " hb-no-mega-menu";
            }
        }
        ...
        if ($depth === 1 && $this->active_megamenu) {
            ...
            if ($this->megamenu_widgetarea == false) {
                ...
                if ($title != "&#8211;" && $title != '"&#8211;"' && $title != "—" && $title != "&#8212;") {
                    ...
                    $item_output .= '<div class="hb-megamenu-title"' . $attributes . '>';
                ...
                if (is_active_sidebar($this->megamenu_widgetarea)) {
                    $item_output .= '<div class="hb-megamenu-widgets-container">';
                    ob_start();
            ...
            if ($depth === 2 && $this->megamenu_widgetarea && $this->active_megamenu) {
                ...
                $item_output .= $args->before;
                $item_output .= '<a class="menu-item-link hb-js-smooth-scroll" ' . $attributes . '>';
                $item_output .= $menu_icon_tag;
